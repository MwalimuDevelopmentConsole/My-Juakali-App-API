...
router.post('/', 
  authenticateToken, 
  authorize(['seller']), 
  uploadMiddleware.products.array('images', 15),
  handleUploadError,
  processUploadedFiles,
  createProduct
);
...
router.delete('/products/:id/images/:imageId',
  ...
  async (req, res) => {
    try {
      ...
      const imageIndex = product.media.images.findIndex(img => img._id.toString() === imageId);
      if (imageIndex === -1) {
        return res.status(404).json({
          success: false,
...
// @access  Seller (own products) or Admin
router.put('/products/:id/images/:imageId/primary',
  authenticateToken,
  authorize(['seller', 'admin']),
  async (req, res) => {
    try {
      ...
      const targetImage = product.media.images.find(img => img._id.toString() === imageId);
      if (!targetImage) {
        return res.status(404).json({
          success: false,
          message: 'Image not found'
