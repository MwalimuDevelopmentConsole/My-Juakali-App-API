...
const validatePreferences = (req, res, next) => {
  ...
  const requiredNotificationTypes = ['newMessage', 'newInquiry', 'urgentUpdates'];
...
router.post('/admin/test', 
  ...
  async (req, res) => {
    try {
      ...
      const { createNotificationData, processNotificationDelivery } = require('../controllers/notificationController');
      ...
      const delivered = await processNotificationDelivery(notificationData, req.io);
...
router.post('/webhooks/email-delivery', async (req, res) => {
  try {
    ...
    const updateData = {
      ...
      'delivery.email.deliveredAt': status === 'delivered' ? new Date(timestamp) : undefined
    };
...
router.post('/webhooks/sms-delivery', async (req, res) => {
  try {
    ...
    const updateData = {
      ...
      'delivery.sms.deliveredAt': status === 'delivered' ? new Date(timestamp) : undefined
    };
