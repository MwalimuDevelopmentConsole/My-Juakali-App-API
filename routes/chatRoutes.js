 ...
 */
router.get('/conversations/:id', validateObjectId('id'), getConversationDetails);
 ...
 */
router.put('/messages/:id/delivered', 
  validateObjectId('id'),
  async (req, res) => {
    try {
  ...
  async (req, res) => {
    try {
      ...
      if (req.io) {
        ...
        handleMessageRead(req.io)(messageId, message.conversation, userId, userType);
      }
...
router.get('/stats', async (req, res) => {
  try {
    ...
    const totalUnreadMessages = conversationsWithUnread.reduce((total, conv) => {
      const participant = conv.participants.find(p => 
        p.user.userId.toString() === userId && 
