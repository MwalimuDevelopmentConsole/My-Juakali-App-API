...
router.get('/tickets', 
  ...
  (req, res, next) => {
    ...
    if (category && category !== 'all') {
      const validCategories = [
        'technical_issue', 'account_verification', 'payment_dispute',
        'business_inquiry', 'product_issue', 'subscription_support',
        'general_support', 'report_abuse'
      ];
      if (!validCategories.includes(category)) {
        return res.status(400).json({
          success: false,
 ...
 * @access  Authenticated admins
 */
router.get('/admin/available-admins', 
  validateAdminAccess,
  async (req, res) => {
    try {
 ...
 */
router.put('/tickets/:id/escalate', 
  validateObjectId('id'),
  validateAdminAccess,
  async (req, res) => {
    try {
...
router.get('/categories', (req, res) => {
  const categories = [
    ...
    { value: 'account_verification', label: 'Account Verification', department: 'customer_support' },
    { value: 'payment_dispute', label: 'Payment Dispute', department: 'finance' },
    { value: 'business_inquiry', label: 'Business Inquiry', department: 'business_development' },
    { value: 'product_issue', label: 'Product Issue', department: 'customer_support' },
    { value: 'subscription_support', label: 'Subscription Support', department: 'customer_support' },
    { value: 'general_support', label: 'General Support', department: 'customer_support' },
    { value: 'report_abuse', label: 'Report Abuse', department: 'content_moderation' }
  ];
