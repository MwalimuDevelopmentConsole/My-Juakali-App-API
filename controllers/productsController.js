      ...
      categoryDoc.dynamicFields.forEach(field => {
        ...
        if (filterValue) {
          switch (field.type) {
            case 'select':
            case 'text':
              filter[`dynamicFields.${field.name}`] = new RegExp(filterValue, 'i');
              break;
            case 'number':
            case 'range':
              if (dynamicFilters[`${field.name}_min`] || dynamicFilters[`${field.name}_max`]) {
                filter[`dynamicFields.${field.name}`] = {};
                if (dynamicFilters[`${field.name}_min`]) {
                  filter[`dynamicFields.${field.name}`].$gte = parseFloat(dynamicFilters[`${field.name}_min`]);
                }
                if (dynamicFilters[`${field.name}_max`]) {
                  filter[`dynamicFields.${field.name}`].$lte = parseFloat(dynamicFilters[`${field.name}_max`]);
                }
              }
              break;
            case 'boolean':
          ...
          rankingScore: {
            $switch: {
              branches: [
                ...
                {
                  case: { $eq: [{ $arrayElemAt: ['$subscriptionPlan.planType', 0] }, 'premium'] },
                  then: 800
                },
                // Pro subscription sellers
                {
                  case: { $eq: [{ $arrayElemAt: ['$subscriptionPlan.planType', 0] }, 'pro'] },
                  then: 900
                },
                // Basic subscription sellers
                {
                  case: { $eq: [{ $arrayElemAt: ['$subscriptionPlan.planType', 0] }, 'basic'] },
                  then: 600
                },
                // Free tier sellers
                {
                  case: { $eq: [{ $arrayElemAt: ['$subscriptionPlan.planType', 0] }, 'free'] },
                  then: 400
                }
              ],
...
const getProduct = async (req, res) => {
  try {
    const product = await Product.findById(req.params.id)
      .populate({
        path: 'seller',
        select: 'firstName lastName businessInfo ratings verification location socialLinks',
        populate: {
    ...
    if (product.status !== 'active' || !product.seller || 
        product.seller.status !== 'active' || !product.seller.isActive) {
      return res.status(404).json({
        success: false,
...
const createProduct = async (req, res) => {
  try {
    ...
    const seller = await Seller.findById(sellerId).populate('currentSubscription');
    ...
    if (seller.currentSubscription) {
      ...
      const subscription = await UserSubscription.findById(seller.currentSubscription)
        .populate('plan');
      
      if (subscription && subscription.subscribedFeatures.maxListings) {
        if (!subscription.subscribedFeatures.unlimitedListings && 
    ...
    // Validation
    if (!title || !description || !type || !primaryCategory || !pricing.basePrice) {
      return res.status(400).json({
        success: false,
        message: 'Required fields: title, description, type, category, and price'
      });
    }
...
const updateProduct = async (req, res) => {
  try {
    ...
    if (title) {
      ...
      product.slug = title.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '') + 
                   '-' + Math.random().toString(36).substr(2, 9);
    }
    ...
    if (keywords) product.keywords = keywords.split(',').map(keyword => keyword.trim());
    if (inventory) product.inventory = { ...product.inventory, ...inventory };
    if (serviceInfo) product.serviceInfo = { ...product.serviceInfo, ...serviceInfo };
