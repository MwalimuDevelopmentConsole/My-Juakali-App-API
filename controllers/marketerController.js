...
const registerMarketer = async (req, res) => {
  try {
    ...
    // Validation
    if (!email || !password || !phone || !firstName || !lastName || !location?.county) {
    ...
    res.status(201).json({
      success: true,
      message: 'Marketer registered successfully. Please verify your email and phone.',
      token,
      marketer: {
        id: marketer._id,
...
const loginMarketer = async (req, res) => {
  try {
    ...
    if (!marketer.isActive || marketer.status === 'banned' || marketer.status === 'suspended') {
...
const getMarketerDashboard = async (req, res) => {
  try {
    ...
    const recentReferrals = await Seller.find({ referredBy: marketerId })
      .sort({ referralDate: -1 })
      .limit(10)
      .select('firstName lastName businessInfo.businessName status referralDate');
  ...
  try {
    ...
    if (req.file) {
      ...
      marketer.avatar = {
        ...
        alt: `${firstName || marketer.firstName} ${lastName || marketer.lastName}`
      };
    }
    ...
    if (paymentInfo) marketer.paymentInfo = { ...marketer.paymentInfo, ...paymentInfo };
    if (communication) marketer.communication = { ...marketer.communication, ...communication };
